...
np.save('data_sdf.npy', trg_sdf)
np.save('data_time.npy', time)
...
def fit_fun(x, trg_sdf, conc, net, p, resfile, plot= False, std= 50.0, repetitions= 5):
    ...
    np.save('model_sdf.npy', model_sdf)
    np.save('error.npy', y)
    if resfile is not None:
        resfile.write(f"{err}\n")
        resfile.flush()        
...
if __name__ == '__main__':
    x0=[4.22825245e-02, 2.15402562e+01, 7.98889005e+00, 1.16413590e-03, -3.15916662e+01, -4.51528381e+00, 8.44657433e-04, -3.34225086e+00, 8.77785161e-01, 9.75556066e-04]
    ...
    res_lsq= minimize(fit_fun,x0, args=(trg_sdf,the_conc,net,p,resfile),bounds=bounds,method='Nelder-Mead',options={'xatol': 1e-2})
