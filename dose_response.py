...
m_conc = 10  # minimum concentration 10^-
s_conc = 1  # concentration step size
trials = 11 # trials / concentration steps
pulse  = 3  # pulse length (s)
clean  = 5  # time between pulses (s)

x = 2.5 + ((pulse+clean)*2)*trials
...
for i in range(trials):  
    ...
    exp_b = (m_conc)
    exp_b = round(exp_b,2)
    conc_a= 10**(-exp_a) + i*10**(-s_conc)
    conc_a_data.append(conc_a)
    print("A conc =",conc_a)
    run(pulse * second, report='text')
    conc_a= 0.0
    run(clean * second, report='text')
    conc_b= 10**(-exp_b) + i*10**(-s_conc)
    conc_b_data.append(conc_b)
    print("B conc =",conc_b)
    run(pulse * second, report='text')
    conc_b= 0.0
    run(clean * second, report='text')
...
# figure()
# plot(trace.t/ms, g_r_a*trace[0].s_r_a*(trace[0].v_a - ENa - trace[0].v_sl),lw=0.2)
...
# figure()
# plot(trace.t/ms, g_M*trace[0].M_a*(trace[0].v_a - EK),lw=0.2)
# plot(trace.t/ms, g_M_b*trace[0].M_b*(trace[0].v_a - EK),lw=0.2)
# xlabel('t (ms)')
# ylabel('"M currents"')
# xlim([0, (x*1000)])
# #ylim([0, 0.65])
# savefig(os.path.join(expt, trial, 'Mcurrent'+'_'+expt+'_'+trial),dpi=600)
...
np.save(os.path.join(expt, trial, 'Mcur_a.npy'), g_M*trace[0].M_a*(trace[0].v_a - EK)/mV)
np.save(os.path.join(expt, trial, 'Mcur_b.npy'), g_M_b*trace[0].M_b*(trace[0].v_a - EK)/mV)
np.save(os.path.join(expt, trial, 'R_a.npy'), g_r_a*trace[0].s_r_a*(trace[0].v_a - ENa - trace[0].v_sl)/mV)
np.save(os.path.join(expt, trial, 'R_b.npy'), g_r_a*trace[0].s_r_b*(trace[0].v_b - ENa - trace[0].v_sl)/mV)
